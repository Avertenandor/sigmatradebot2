...
async def _show_dashboard(message: Message, state: FSMContext) -> None:
    ...
    if has_cold:
        text += (
            ...
            f"Адрес: `{escape_md(cold_address)}`\n"
...
async def input_address(message: Message, state: FSMContext):
    ...
    await safe_answer(
        message,
        f"📤 **Отправка {currency}**\n"
        f"Получатель: `{escape_md(address)}`\n\n"
...
async def _show_confirmation(message: Message, state: FSMContext):
    ...
    await safe_answer(message, text, parse_mode="Markdown", reply_markup=wallet_confirm_keyboard())
    ...
    try:
        ...
        if result["success"]:
            await safe_answer(
                ...
                f"🔗 Hash: `{escape_md(result['tx_hash'])}`\n\n"
