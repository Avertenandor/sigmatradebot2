...
**Цель:** Проверить готовность бота к production под реалистичной нагрузкой.

**Шаги:**

1. **Подготовка:**
   - Развернуть на staging с ресурсами, близкими к production
   - Настроить мониторинг: `/health`, логи, RPC stats, Redis memory
   - Убедиться, что все сервисы запущены (bot, worker, scheduler)

2. **Выполнение:**
   - Симулировать 1000 одновременных пользователей
   - Каждый пользователь выполняет операции: регистрация, депозиты, выводы
   - Длительность: 30 минут
   - Мониторить в реальном времени: `/health`, логи, метрики

3. **Проверка результатов:**
   - p95 время ответа < 500ms
   - Частота ошибок < 0.1%
   - Нет ошибок RPC rate limit
   - Security логи читаемы и структурированы
   - RPC stats показывают нормальную нагрузку (< 25 RPS)
   - Redis память стабильна
